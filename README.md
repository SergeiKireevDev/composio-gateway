...
3. In **Tool permissions**, filter by member to see their connected apps at the last sync, then enable the tools you need and **Save permissions**. All newly discovered tools are **disabled by default**, including tools added by later scans. Member selection only filters the view: the saved policy remains shared by all members. Existing permissions are preserved on upgrade, refresh, restart, and app reconnect; this change does not reset previously available tools. Connected members cannot obtain an executable session until at least one of their tools is enabled.
